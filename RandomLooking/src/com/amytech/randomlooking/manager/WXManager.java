...
import com.amytech.randomlooking.parser.WXParser;
...
public class WXManager extends BaseManager {
	...
	public interface WXGetCallback {
		void wxGetSuccess(List<WXModel> result);

		void wxGetFailure();
	}

	private static final String URL = "http://api.huceo.com/wxnew/?key=e49756dd8491033290771e5661221f8e&num={0}&rand=1";
	...
	public void load(final int count, final WXGetCallback callback) {
		CLIENT.get(MessageFormat.format(URL, count), new JsonHttpResponseHandler("UTF-8") {
			@Override
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				super.onSuccess(statusCode, headers, response);
				if (callback != null) {
					callback.wxGetSuccess(WXParser.parse(response, count));
				}
			}

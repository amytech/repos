...
package com.amytech.android.library.views.imagechooser.api;
...
import com.amytech.android.library.BuildConfig;
import com.amytech.android.library.views.imagechooser.threads.ImageProcessorListener;
import com.amytech.android.library.views.imagechooser.threads.ImageProcessorThread;
import com.amytech.android.library.views.imagechooser.threads.MediaProcessorThread;
import com.amytech.android.library.views.imagechooser.threads.VideoProcessorListener;
import com.amytech.android.library.views.imagechooser.threads.VideoProcessorThread;
...
public class MediaChooserManager extends BChooser implements ImageProcessorListener, VideoProcessorListener {
	...
	public MediaChooserManager(android.app.Fragment fragment, int type, String foldername) {
		super(fragment, type, foldername, true);
	}
	 ...
	 */
	public MediaChooserManager(Activity activity, int type, boolean shouldCreateThumbnails) {
		super(activity, type, shouldCreateThumbnails);
	}

	public MediaChooserManager(Fragment fragment, int type, boolean shouldCreateThumbnails) {
		super(fragment, type, shouldCreateThumbnails);
	}

	public MediaChooserManager(android.app.Fragment fragment, int type, boolean shouldCreateThumbnails) {
		super(fragment, type, shouldCreateThumbnails);
	}
	 ...
	 * @param shouldCreateThumbnails
	 */
	public MediaChooserManager(Activity activity, int type, String foldername, boolean shouldCreateThumbnails) {
		super(activity, type, foldername, shouldCreateThumbnails);
	}

	public MediaChooserManager(Fragment fragment, int type, String foldername, boolean shouldCreateThumbnails) {
		super(fragment, type, foldername, shouldCreateThumbnails);
	}

	public MediaChooserManager(android.app.Fragment fragment, int type, String foldername, boolean shouldCreateThumbnails) {
		super(fragment, type, foldername, shouldCreateThumbnails);
	}
	...
	public String choose() throws Exception {
		...
		case ChooserType.REQUEST_PICK_PICTURE_OR_VIDEO:
			...
			break;
		default:
			throw new IllegalArgumentException("This chooser type is unappropriate with MediaChooserManager: " + type);
		}
		return path;
	}
		...
		if (data != null && data.getDataString() != null) {
			...
			if (filePathOriginal == null || TextUtils.isEmpty(filePathOriginal)) {
				...
				if (!wasVideoSelected(data)) {
					thread = new ImageProcessorThread(path, foldername, shouldCreateThumbnails);
					((ImageProcessorThread) thread).setListener(this);
				} else {
					thread = new VideoProcessorThread(path, foldername, shouldCreateThumbnails);
